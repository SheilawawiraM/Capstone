...
void initializeRoutes(BusRoute routes[]) {
    strcpy(routes[0].destination, "meru");
    routes[0].available_seats = MAX_SEATS;

    strcpy(routes[1].destination, "embu");
...
int processPayment(Reservation *reservation, float amount) {
    ...
    printf("Processing payment of %.2f for reservation ID %d.\n", amount, reservation->seatNumber);
    ...
    if (authenticateUser(username, password)) {
        ...
        do {
            ...
            switch (choice) {
                ...
                case 2:
                    printf("Select route (1 for meru, 2 for embu): ");
                    scanf("%d", &routeChoice);
